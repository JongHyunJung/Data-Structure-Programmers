...
public class Solution4 {
	...
	public int solution1(int[] nums){
		int maxNum = nums.length / 2; //배열 크기의 1/2

		//폰켓몬의 종류와 갯수
		Map<Integer, Integer> phonekemon = new HashMap<>();
		for(int i : nums) phonekemon.put(i, 0);

		int num = phonekemon.keySet().size();
		
		//최소값을 선택한다.
		return Math.min(maxNum, num);
		//return Math.min(phonekemon.keySet().size(), nums.length / 2);
		//return num < maxNum ? num : maxNum;
	}
	
	public int solution2(int[] nums) {
		List<Integer> phonekemon = new LinkedList<>();
		
		for(int n : nums) {
			if(!phonekemon.contains(n)) phonekemon.add(n);
		}
		int answer = phonekemon.size();
		int maxNum = nums.length/2;
		
		return Math.min(answer, maxNum);
	}
} 
